<template>
  <div v-if="article">
    <UtilsBanner :url="functionalCookieAccepted ? article.attributes.cover.data.attributes.url : false"
      :icon="!functionalCookieAccepted ? 'mdi-cookie-alert' : false" :strapiImg="functionalCookieAccepted" />
    <v-container style="margin-top: -45px">
      <v-avatar color="white"><v-icon>{{
      ...
      <div v-if="functionalCookieAccepted">
        <iframe v-if="article.attributes.spotifylink" :src="
          'https://open.spotify.com/embed/' +
          article.attributes.spotifylink.replace(
            'https://open.spotify.com/',
            ''
          )
        " data-cookiescript="accepted" :alt="altText" :data-alt-img="altImg" data-cookiecategory="functionality"
          class="pt-2" width="100%" frameBorder="0" allowtransparency="true" allow="encrypted-media"></iframe>
        <div v-if="article.attributes.iframe" v-html="article.attributes.iframe"></div>
        <iframe v-if="article.attributes.youtubelink"
          :src="'https://www.youtube.com/embed/' + article.attributes.youtubelink.replace('https://youtu.be/', '')"
          data-cookiescript="accepted" :alt="altText" :data-alt-img="altImg" data-cookiecategory="functionality"
          width="100%" class="pt-2" title="YouTube video player" frameborder="0"
      ...
      >
        ...
        <iframe v-if="article.attributes.soundcloudlink" :src="
        ...
        " data-cookiescript="accepted" :alt="altText" :data-alt-img="altImg" data-cookiecategory="functionality"
          class="pt-2" width="100%" height="300" scrolling="no" frameborder="no" allow="autoplay"></iframe>
      </div>
      <v-banner v-else>
        <v-icon>mdi-cookie-alert</v-icon> {{ altText }}
      </v-banner>
      <div class="singleArticleText mt-8" :style="cssVars" v-html="$md.render(article.attributes.text)"></div>
    </v-container>
    <v-divider />
    <v-container>
      <h2 class="text-h2 secondary--text mb-4 mt-12">Neueste Einträge</h2>
      <v-row v-if="relatedArticles" class="pt-8" style="z-index: 1">
        <v-col v-for="relatedArticle in relatedArticles.filter(
          (m) => m.attributes.tags.data[0].attributes.name != slugpath
        )" :key="relatedArticle.attributes.id" cols="12" sm="6" md="4" class="pb-4">
