  ...
  <div v-if="!postponed && !activateVideoLink.done">
    <v-expansion-panel-header color="grey lighten-5" class="d-flex align-center justify-start">
      ...
      <div v-if="item.acceptedDate">
        <v-chip v-if="new Date(item.acceptedDate) <= new Date()" class="ma-2 flex-shrink-0 flex-grow-1"
          color="success" text-color="white">
          laufend...
        </v-chip>
        <v-chip v-else class="ma-2 flex-shrink-0 flex-grow-1" color="success" text-color="white">
          bereit
        </v-chip>
      </div>

      <div v-else-if="item.coachAnswered">
        <v-chip class="ma-2 flex-shrink-0 flex-grow-1" color="orange" text-color="white">
          Antwort abwarten
        </v-chip>
      </div>
      <div v-else>
        <v-chip class="ma-2 flex-shrink-0 flex-grow-1" color="red" text-color="white">
          unbearbeitet
        </v-chip>
      </div>
...
export default {
  ...
  computed:{
    activateVideoLink(){
      if(this.item.acceptedDate){
        ...
        return {
          ready: overTime >= now && now >= preTime,
          done: now > overTime,
        }
      }
      return {
  ...
  methods: {
    ...
    cancel(doc) {
      ...
      this.$deleteMeeting(this.user.email, this.id, doc.acceptedDate, doc.paymentID)
        .then((r) => {
          this.isDelete = false;
