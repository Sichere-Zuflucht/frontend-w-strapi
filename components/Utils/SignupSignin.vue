			...
			<v-container style="max-width: 450px" class="ma-auto py-16">
				...
				<v-stepper v-model="step" :flat="true" style="box-shadow: none">
					<v-stepper-items>
						<v-stepper-content v-if="!makeLogin" step="1" class="pa-0 mt-4">
							<h2 v-if="!emailExisting" class="text-h3 secondary--text">
								Ihr Anliegen
							</h2>
						...
						<v-stepper-content step="3" class="pa-0">
							<v-form
								...
								class="mb-8"
								autocomplete="on"
								@submit.prevent="emailExisting ? login() : register()"
							>
								<v-alert
								...
								<div v-else>
									<h2
										v-if="!emailExisting"
										class="text-h3 secondary--text mt-4"
									>
										Account erstellen
									</h2>
									<v-text-field
									...
									<v-text-field
										...
										:autocomplete="
											emailExisting ? 'current-password' : 'new-password'
										"
										:hint="
											emailExisting
												? ''
									...
									<div class="d-flex justify-end">
										...
										><v-btn
											...
											>{{
												emailExisting ? 'Einloggen' : 'Account erstellen'
											}}</v-btn
										>
									</div>
									<v-alert v-if="codeSent.status" type="success" class="mt-2">{{
										codeSent.message
									}}</v-alert>
								</div>
							</v-form>
				...
				>
					{{
						error.message
							? error.message
							: `Es wurde eine E-Mail inklusive einem Link an ${email} geschickt. Bitte
